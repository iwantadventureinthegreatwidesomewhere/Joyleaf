...
exports.get_content = functions.https.onRequest((request, response) => {
    ...
    admin.auth().getUser(uid).then(() => {
        admin.database().ref().child('content').once('value').then((snapshot) => {
            var categories = [
                ['Relaxed', 'Cannabis can relax both the body and the mind when looking to wind down at the end of a long day.'],
                ['Hungry', 'Grumbles in your tummy? These strains can help naturally kick-start appetite.'],
                ['Happy', 'It\'s said that true happiness is a state of mind. These strains can help induce intense feelings of joy.'],
                ['Creative', 'Expand your imagination with strains that can stimulate new creative thoughts and ideas.'],
                ['Energetic', 'Keep active with strains that can crush fatigue.'],
                ['Social', 'Liven up social events with strains that can help reduce social anxiety and provide talkative highs.'],
                ['Sleepy', 'Sleep is essential for maintaining health. Cannabis can be an effective treatment for sleep disorders and difficulties.'],
                ['Focused', 'Get more done faster with strains that can improve focus.'],
                ['Giggly', 'Find the perfect companion strain to enjoy with your favorite music, movies and shows.'],
                ['Aroused', 'Bring some bud into the bedroom with strains that can heighten sexual experiences.'],
            ];
            ...
            for(var i = 0; i < 7; i++){
                ...
                if(category[0] == 'Relaxed' || category[0] == 'Hungry' || category[0] == 'Happy' || category[0] == 'Creative' || category[0] == 'Energetic' || category[0] == 'Sleepy' || category[0] == 'Focused' || category[0] == 'Giggly' || category[0] == 'Aroused'){
            ...
            Promise.all(ratedCuratedItems).then(function(ratedCuratedItems){
                Promise.all(ratedFeaturedList).then(function(ratedFeaturedList){
                    ...
                    response.json({ curated : ratedCuratedList, featured : ratedFeaturedList });
                });
            });
        });
    });
...
exports.post_review = functions.https.onRequest((request, response) => {
    ...
    admin.auth().getUser(uid).then(() => {
        ...
        admin.database().ref().child('reviews').child(obj.id).once('value').then((snapshot) => {
            var date = new Date();

            admin.database().ref().child('users').child(uid).once('value').then((snapshot) => {
                var name = snapshot.child('name').val();

                admin.database().ref().child('reviews').child(obj.id).child('data').push({
                    poster_name : name,
                    score : parseFloat(obj.rating),
                    review : obj.review,
                    timestamp : ('' + (date.getMonth()+1) + '/' + date.getDate() + '/' + date.getFullYear())
                }).then((snapshot) => {
                    admin.database().ref().child('users').child(uid).child('reviews').push({
            ...
            if(snapshot.exists()){
                ...
                admin.database().ref().child('reviews').child(obj.id).update({
                    averageRating : ((averageRating * (numberOfReviews)) + parseFloat(obj.rating)) / (numberOfReviews + 1),
...
exports.get_reviews = functions.https.onRequest((request, response) => {
    ...
    admin.auth().getUser(uid).then(() => {
        admin.database().ref().child('reviews').child(strain_id).once('value').then((snapshot) => {
            if(snapshot.exists()){
                admin.database().ref().child('reviews').child(strain_id).child('data').limitToLast(25).once('value').then((ratings) => {
                    var averageRating = snapshot.child('averageRating').val();
                    var numberOfReviews = snapshot.child('numberOfReviews').val();
                    var ratings = ratings.val();

                    response.json({ averageRating : averageRating, numberOfReviews : numberOfReviews, ratings : ratings });
                });
            }else{
                response.json({ averageRating : 0, numberOfReviews : 0, ratings : {} });
            }
        });
    });
});
        ...
        admin.database().ref().child('content').once('value').then((snapshot) => {
            ...
            shuffle(items).forEach(function(item){
                ...
                for(var word of words){
                    if(item.child('name').val().toLowerCase().includes(word.toLowerCase())){
                        match = true;
                        priority += 10;
                    }
                    ...
                    var positiveEffects = item.child('effects').child('positive').val();
                    ...
                    var negativeEffects = item.child('effects').child('negative').val();
        ...
        admin.database().ref().child('users').child(uid).once('value').then((snapshot) => {
            for(var key in topics){
                if(snapshot.child('topics').child(key).val() != null){
                    admin.database().ref().child('users').child(uid).child('topics').update({
                        [key] : (snapshot.child('topics').child(key).val()) + topics[key]
                    });
                }else{
                    admin.database().ref().child('users').child(uid).child('topics').update({
                        [key] : topics[key]
            ...
            for(var key in tags){
                if(snapshot.child('tags').child(key).val() != null){
                    admin.database().ref().child('users').child(uid).child('tags').update({
...
exports.highfive = functions.https.onRequest((request, response) => {
    ...
    admin.auth().getUser(uid).then(() => {
        admin.database().ref().child('users').child(uid).once('value').then((user) => {
            var topics = [];
            var tags = [];
            ...
            admin.database().ref().child('content').once('value').then((content) => {
                var items = [];
                ...
                items.forEach(function(item){
                    ...
                    for(var topic of topics){
                        ...
                        if(flavors != null){
                            flavors.forEach(function(flavor){
                                if(flavor.toLowerCase().includes(topic.key.toLowerCase())){
                                    match = true;
                                }
                            });
                        }
                        ...
                        if(positiveEffects != null){
                            positiveEffects.forEach(function(effect){
                                if(effect.toLowerCase().includes(topic.key.toLowerCase())){
                                    match = true;
                                }
                            });
                        }
                    ...
                    for(var tag of tags){
                        ...
                        if(flavors != null){
                            flavors.forEach(function(flavor){
                                if(flavor.toLowerCase().includes(tag.key.toLowerCase())){
                                    match = true;
                                }
                            });
                        }
                        ...
                        if(positiveEffects != null){
                            positiveEffects.forEach(function(effect){
                                if(effect.toLowerCase().includes(tag.key.toLowerCase())){
                                    match = true;
                                }
                            });
                        }
                ...
                Promise.all(rankings.map(function(ranking) { return ranking[0] })).then(function(resolved){
                    var result = [];
...
function getReviews(item){
    ...
    return admin.database().ref().child('reviews').child(strain_id).once('value').then((snapshot) => {
        if(snapshot.exists()){
            return admin.database().ref().child('reviews').child(strain_id).child('data').limitToLast(25).once('value').then((ratings) => {
                var averageRating = snapshot.child('averageRating').val();
                var numberOfReviews = snapshot.child('numberOfReviews').val();
                var ratings = ratings.val();

                obj.reviews = { averageRating : averageRating, numberOfReviews : numberOfReviews, ratings : ratings };
                return obj;
            });
        }else{
